...
!
! !INTERFACE:
subroutine read_merra2(n, order, month, findex, slvname, flxname, &
                       lfoname, radname, merraforc, ferror)
! !USES:
...
! Read in the flux file fields (*flx):
  ...
  if(file_exists) then 
   ...
   ! Read in the *corrected* total precipitation field: !LB adapted for SE
